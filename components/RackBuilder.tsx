...
// Utility functions inline since we need them
const calculateTotalWeight = (gear, rackType, rackUnits) => {
  const gearWeight = gear.reduce((total, item) => total + (item.weight || 5), 0)
  const rackWeight = rackType === "fly" ? rackUnits * 2 : rackUnits * 3
  const totalLbs = gearWeight + rackWeight
  return { totalLbs, totalKg: totalLbs * 0.453592 }
}

const getWeightWarnings = (gear, rackUnits, rackType) => {
  const warnings = []
  const { totalLbs } = calculateTotalWeight(gear, rackType, rackUnits)
  if (totalLbs > 100) warnings.push("Heavy rack - consider weight distribution")
  return warnings
}

const useRackConfig = (rack) => {
  return {
    isValidRack: !!rack,
    rackType: rack?.type || "fly",
    rackSizeU: rack?.units || 4,
  }
}

const findEmptySlotInChassis = (chassis) => {
  if (!chassis.modules) return -1
  return chassis.modules.findIndex((slot) => slot === null)
}

const addModuleToChassis = (chassis, module, slotIndex) => {
  const newModules = [...chassis.modules]
  newModules[slotIndex] = module
  return { ...chassis, modules: newModules }
}

const removeModuleFromChassis = (chassis, slotIndex) => {
  const newModules = [...chassis.modules]
...
export default function RackBuilder({ project, setProject }) {
  ...
  const normalizedProject = {
    ...
    racks: project.racks.map((rack) => ({
      ...
      gear: rack.gear || [...(rack.frontGear || []), ...(rack.backGear || [])],
  ...
  // Add new rack
  const handleAddRack = (rackConfig) => {
    // Parse rack configuration
    let units, type, name

    if (typeof rackConfig === "string") {
      // Legacy string format
      const parts = rackConfig.split(" ")
      units = Number.parseInt(parts[0].replace("U", ""))
      type = parts.slice(1).join(" ")
      name = rackConfig
    } else {
      // New object format
      units = Number.parseInt(rackConfig.size.replace("U", ""))
      type = `${rackConfig.type} ${rackConfig.width} ${rackConfig.depth}`
      name = `${rackConfig.size} ${rackConfig.type} ${rackConfig.width} ${rackConfig.depth}`
    }
    ...
    const newRack = {
      id,
      name,
      type: rackConfig.type?.includes("Fly") ? "fly" : "tour",
      units,
      width: rackConfig.width || "single",
      gear: [],
      frontGear: [],
      backGear: [],
      notes: "",
  ...
  const handleAddCustomItem = (customItem) => {
    ...
    const gear =
      typeof customItem === "string"
        ? {
            name: customItem,
            units: Number.parseInt(customItem.charAt(0)) || 1,
            category: "Custom",
            color: "#666666",
          }
        : customItem
        ...
        {normalizedProject.racks.length === 0 && (
          <div className="flex items-center justify-center h-96">
            <div className="text-center">
              ...
              <button onClick={handleBackToProjects} className="mt-4 text-blue-400 hover:text-blue-300 underline">
                ← Back to Projects
              </button>
            </div>
          </div>
